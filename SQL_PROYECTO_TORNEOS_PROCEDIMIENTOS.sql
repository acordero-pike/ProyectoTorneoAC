...
CREATE PROC BG_Insertar_Cancha  @Nombre varchar(100),@TipoCancha varchar(50),@Disponibilidad varchar(50)
AS
insert into cancha(Nombre,TipoCancha,Disponibilidad)values(@Nombre,@TipoCancha,@Disponibilidad);
go


--modifica Cancha
create proc  BG_Editar_Cancha @NumeroCancha int,@Nombre varchar(100),@TipoCancha varchar(50),@Disponibilidad varchar(50)
as
update cancha set Nombre=@Nombre,TipoCancha=@TipoCancha,Disponibilidad=@Disponibilidad where NumeroCancha =@NumeroCancha
go
...
@email varchar(100), @rol varchar(100)
as insert into arbitro(DPI, Nombres, Apellidos, Direccion, Telefono, Nacionalidad, FechaNac, Correo, Rol) values(@DPI, @nombre, @apellido,
@direccion, @telefono, @nacionalidad, @fechaNac, @email, @rol);
go
...
FechaNac = @fechaNac, Correo = @email, Rol = @rol where DPI = @DPI;
go
